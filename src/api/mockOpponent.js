...
// 🕛 ฟังก์ชันช่วย: สร้าง timestamp (วินาที) ในช่วง 12:00 - 13:00 ของ "วันนี้"
function randomTimeBetween12to13() {
  const baseDate = new Date(); // วันที่วันนี้
  baseDate.setHours(12, 0, 0, 0); // เริ่มที่ 12:00:00
  const start = baseDate.getTime();
  const end = start + 60 * 60 * 1000; // +1 ชั่วโมง
  const randomMs = Math.floor(Math.random() * (end - start) + start);
  return Math.floor(randomMs / 1000); // คืนค่าเป็นวินาที
}

// 🚁 ฟังก์ชันสร้างข้อมูลจำลองโดรนฝ่ายตรงข้าม
function generateOpponentData(deviceId, type = "danger", count = 1) {
  const data = [];
  const baseLat = 13.736;
  const baseLon = 100.523;
  const baseTime = randomTimeBetween12to13() * 1000; // ✅ ใช้ช่วงเวลา 12:00–13:00
  ...
  for (let i = 0; i < count; i++) {
    data.push({
      ...
      type: type,
      timestamp: new Date(baseTime + (i * 10000)).toISOString() // 🕒 readable time
    });
  }
...
async function saveToMongoDB(dataArray) {
  try {
    ...
    const coll = db.collection('opponents');
    ...
    dataArray.forEach((item, index) => {
      console.log(`  ${index + 1}. ${item.deviceId} [${item.type}]`);
      console.log(`     🌐 (${item.latitude.toFixed(6)}, ${item.longitude.toFixed(6)})`);
...
async function generateMockOpponentData() {
  ...
  const mockData = [
    ...generateOpponentData("OPPONENT-001", "danger", 3),
    ...generateOpponentData("OPPONENT-002", "warning", 2),
    ...generateOpponentData("OPPONENT-003", "danger", 2),
